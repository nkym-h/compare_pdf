...
def mark_differences(img1_path, img2_path, output_path, mark_color=(0, 0, 255), tolerance=10):
    ...
    for contour in contours:
        ...
        if w > tolerance or h > tolerance:
            cv2.rectangle(img1, (x, y), (x + w, y + h), mark_color, 2)
            ecout += 1

    # テキストの差分を検出
    text1 = extract_text_from_image(img1_path)
    text2 = extract_text_from_image(img2_path)
    text_diff = list(unified_diff(text1.splitlines(), text2.splitlines()))
...
def get_param():
    ...
    parser.add_argument("--mark_color", type=str, help="Color for marking differences in BGR format.", default="0,0,255")
    parser.add_argument("--tolerance", type=int, help="Tolerance for ignoring shifts.", default=10)
...
def main():
    ...
    pdf1_path = args.pdf1
    pdf2_path = args.pdf2
    mark_color = tuple(map(int, args.mark_color.split(',')))
    ...
    for page_num, (img1, img2) in enumerate(zip(images_pdf1, images_pdf2), start=1):
        ...
        if mark_differences(img1_path, img2_path, output_path, mark_color, tolerance) > 0:
